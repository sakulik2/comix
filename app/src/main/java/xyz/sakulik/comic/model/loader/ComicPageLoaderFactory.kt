...
class ComicPageLoaderFactory(private val context: Context) {
    ...
    private val apiService by lazy {
        ...
        val baseUrlFromSettings = runBlocking { 
            SettingsDataStore.getComicApiBaseUrlFlow(context).firstOrNull() 
    ...
    fun create(comic: ComicEntity): ComicPageLoader {
        return when (comic.source) {
            ...
            ComicSource.REMOTE -> {
                val baseUrl = runBlocking { SettingsDataStore.getComicApiBaseUrlFlow(context).firstOrNull() } ?: "https://comix.sakulik.xyz/"
